...
def agent_orchestrator_router(state_updates: dict) -> str:
    return state_updates["route_decision"]
...
graph.add_edge("fetch_bot_settings", "sentiment_analyzer")
graph.add_edge("sentiment_analyzer", "fetch_previous_messages")
graph.add_edge("fetch_previous_messages", "check_cache")

graph.add_conditional_edges("check_cache", cache_router, {
    "hit": "set_cached_response_as_output",
    "miss": "agent_orchestrator"
})

graph.add_conditional_edges("agent_orchestrator", agent_orchestrator_router, {
    "off_topic": "off_topic",
    "continue": "cache_response"
})
